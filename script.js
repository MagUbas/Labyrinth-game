...
function drawMaze(){ // using randomized Prim's algorithm
    ...
    if (canvas.getContext) { // checking if canvas is working 
        let grid = [];
        // making of matrix
        ...
        ctx.fillStyle = 'white';
        ctx.fillRect(numberX*wallSize+ 0.1*wallSize,numberY*wallSize+0.1*wallSize,wallSize*0.8,wallSize*0.8);
        
        function neighbors(){ // find neighbors of current cell
                ...
                switch(ArrDirection[i]){
                    case 0:
                        if(posX!==(numBlock-1) && grid[posX+1][posY].value===0){
                            ctx.fillRect(posX*wallSize + 0.1*wallSize,posY*wallSize+ 0.1*wallSize,wallSize*1.2,wallSize*0.8);
                            i=4;
                        };
                        break;
                    case 1:
                        if(posX!==0 && grid[posX-1][posY].value===0){
                            ctx.fillRect(posX*wallSize - 0.1*wallSize,posY*wallSize+ 0.1*wallSize,wallSize,wallSize*0.8);
                            i=4;
                        };
                        break;
                    case 2:
                        if(posY!==(numBlock-1) && grid[posX][posY+1].value===0){
                            ctx.fillRect(posX*wallSize + 0.1*wallSize,posY*wallSize+ 0.1*wallSize,wallSize*0.8,wallSize*1.2);
                            i=4;
                        };
                        break;
                    case 3:
                        if(posY!==0 && grid[posX][posY-1].value===0){
                            ctx.fillRect(posX*wallSize + 0.1*wallSize,posY*wallSize- 0.1*wallSize,wallSize*0.8,wallSize);
                            i=4;
                        };
                        break;
...
function startGame(){
    ...
    end.style.left = Math.floor(0.25*wallSize + (numBlock-1)*wallSize) +'px';
    
    end.style.top = Math.floor(0.25*wallSize + (numBlock-1)*wallSize) +'px';
    ...
    document.onkeydown = function(e) { // checking if key was pressed
        switch (e.keyCode) {
            case 37:
            case 65:
                //alert('left');
                
                colorArr = ctx.getImageData(parseInt(player.style.left, 10)-A, parseInt(player.style.top, 10), A, A).data; 
            ...
            case 87:
                //alert('up');
                colorArr = ctx.getImageData(parseInt(player.style.left, 10), parseInt(player.style.top, 10)-A, A, A).data; 
                for(let i=0; i<colorArr.length;i++){
                    if(colorArr[i]===0){
                        deadEnd=true;
                    }    
    ...
    document.onkeyup = function(){
        ...
        if(player.style.left===end.style.left && player.style.top===end.style.top){
            alert('Win');
            drawMaze();
            // przy 20 cos sie rozjezdza, chuj wie czemu
            }
    }
}